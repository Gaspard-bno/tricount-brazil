...
// Write access still goes through a code-checking, optimistic-locking RPC.
...
const CACHE_KEY = "tricount-brazil-state-v3";
const BASE_KEY = "tricount-brazil-sync-base-v3";
...
async function fetchRemoteRow() {
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/trip_state?id=eq.${TRIP_ID}&select=id,state,version,updated_at,updated_by`,
...
async function syncPending(eventType = "Synchronisation", actorId = "") {
  ...
  isSaving = true;
  setSync("saving", "Synchronisation…");
  try {
    ...
    if (!pending) {
      if (Number(remoteRow.version) >= remoteVersion) {
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/save_brazil_trip_state`, {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      if (/CONFLICT_VERSION/i.test(message)) {
        isSaving = false;
        return syncPending(eventType, actorId);
      }
    ...
    setSync(offline ? "offline" : "error", offline ? "Hors ligne" : "À resynchroniser");
    if (!offline) toast(error.message || "Synchronisation impossible.", "error");
  } finally {
    isSaving = false;
  }
}
...
async function loadRemote() {
  ...
  try {
    ...
    if (localStorage.getItem(PENDING_KEY)) {
      await syncPending("Retour en ligne", state.updatedBy);
      return;
    }
    persistSynced(row.state, row.version);
    render();
...
function startRealtime() {
  ...
  realtimeClient
    ...
    .on(
      "postgres_changes",
      { event: "UPDATE", schema: "public", table: "trip_state", filter: `id=eq.${TRIP_ID}` },
      (payload) => {
        if (isSaving || Number(payload.new.version) <= remoteVersion) return;
        if (localStorage.getItem(PENDING_KEY)) {
          void syncPending("Fusion après mise à jour distante", state.updatedBy);
          return;
        }
        persistSynced(payload.new.state, payload.new.version);
        setSync("live", "Mis à jour");
        render();
        toast("Mise à jour reçue de l’autre appareil.");
      },
    )
    .subscribe();
}
...
async function getLatestRate({ force = false } = {}) {
  ...
  try {
    ...
    const response = await fetch(RATE_URL, { signal: controller.signal, cache: "no-store" });
