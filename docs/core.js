...
export function normalizeState(input) {
  ...
  const hasPermanentMembers = RESIDENT_IDS.every((id) =>
  ...
  return {
    ...
    schemaVersion: SCHEMA_VERSION,
    tripId: TRIP_ID,
    members: hasPermanentMembers ? members : fallback.members,
    spaces: array("spaces").some((space) => space?.id === APARTMENT_ID)
      ? array("spaces")
      : fallback.spaces,
...
export function calculateShares({ amountEur, participantIds, mode = "equal", values = {} }) {
  ...
  if (!(total > 0)) throw new Error("Le montant converti doit être positif.");

  if (mode === "equal") {
    const base = Math.floor((total * 100) / ids.length) / 100;
    let remainder = roundMoney(total - base * ids.length);
    return ids.map((memberId) => {
      const extra = remainder >= 0.01 ? 0.01 : 0;
      remainder = roundMoney(remainder - extra);
      return { memberId, amountEur: roundMoney(base + extra) };
    });
  }
  ...
  if (mode === "exact") {
    ...
    if (shares.some((share) => share.amountEur < 0)) {
    ...
    return ids.map((memberId, index) => {
      const amount =
        index === ids.length - 1
          ...
          : roundMoney((total * percentages[index]) / 100);
      assigned = roundMoney(assigned + amount);
      return { memberId, amountEur: amount };
    });
  }
